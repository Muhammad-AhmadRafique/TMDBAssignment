...
class PostAPIGeneric<request: Codable,response: Codable> {
    ...
    static func postRequest(apiURL:String, requestModel:request, requestType: RequestType = .post, onCompletion: @escaping (Result<response,Error>) -> Void) {

        guard let parameters = requestModel.dictionary else { return }
        
//        print(parameters.convertToJSONString())
        NetworkService.shared.fetchData(requestType: requestType, fromURL: apiURL, parameters: parameters) { (error:Error?, jsonData:Data?, statusCode:Int?) in
            if let err = error{
                let errr = CustomError(description: err.localizedDescription)
                onCompletion(.failure(errr))
            }else{
